...
int32_t main()
{
    ...
    test
    {
        ...
        string rem = s.substr(l, n - 2 * l);// l+.....+l ==> n-2l

        string ans = s.substr(0, l);//pushing first l which is palindrom with last l 

        if (rem.size())
        {
            int l_pre = largest_pre_pal(rem);// finding largest palindrom prefix 

            reverse(rem.begin(), rem.end());
            int l_suf = largest_pre_pal(rem); // finding largest palindrom suffix
